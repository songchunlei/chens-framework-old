...
//                }
    ...
    public void fetchProcessDefinitionEntity() {
        ...
        GetDeploymentProcessDefinitionCmd cmd = new GetDeploymentProcessDefinitionCmd(
                processDefinitionId);
        processDefinitionEntity = (ProcessDefinitionEntity)cmd.execute(Context.getCommandContext());
    }
    ...
    public void fetchHistoricActivityInstances() {
        ...
        Collections.sort(historicActivityInstances, new Comparator<HistoricActivityInstance>() {

			@Override
			public int compare(HistoricActivityInstance o1, HistoricActivityInstance o2) {
				// TODO Auto-generated method stub
				if("startEvent".equals(o2.getActivityType())){
					return 1;
				}
    ...
    public Edge findPreviousEdge(Node currentNode, long currentStartTime) {
        ...
        BpmnModel bpmnModel = Context.getProcessEngineConfiguration().getRepositoryService().getBpmnModel(processDefinitionEntity.getId()); //获取流程xml模型
        FlowNode activityImpl = (FlowNode) bpmnModel.getMainProcess().getFlowElement(activityId); //获取历史节点
...
//                .findActivity(activityId);
        ...
        // 遍历进入当前节点的所有连线
        for (SequenceFlow sequenceFlow : activityImpl.getIncomingFlows()) {
        	FlowNode source = (FlowNode)sequenceFlow.getSourceFlowElement();
