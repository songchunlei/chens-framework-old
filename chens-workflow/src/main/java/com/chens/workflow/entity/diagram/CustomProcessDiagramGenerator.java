package com.chens.workflow.entity.diagram;
...
public class CustomProcessDiagramGenerator {
    ...
    public static final int OFFSET_TASK = 5;//20  圆角弧度
    ...
    private static Color RUNNING_COLOR = new Color(236,81,89);//Color.RED;
    private static Color HISTORY_COLOR = new Color(146,196,38);//new Color(0,191,255);//Color.BLUE;
    private static Color SKIP_COLOR = Color.GRAY;
    private static Stroke THICK_BORDER_STROKE = new BasicStroke(3.0f);
    private static int SEQUENCE_FLOW_RADIUS = 0;
    private int minX;
    private int minY;
    ...
    public InputStream generateDiagram(String processInstanceId)
        ...
        HistoricProcessInstance historicProcessInstance = Context
                .getCommandContext().getHistoricProcessInstanceEntityManager().findById(processInstanceId);
        ...
        this.minX -= 5;
        this.minY -= 5;*/
        //以上代码做了 图形最小化功能，就是把左上的控件给减了 注释了不要 直接原样输出
        this.minX = 0;
        this.minY = 0;
        ProcessDefinition definition = new GetDeploymentProcessDefinitionCmd(
        ...
        List<HistoricActivityInstance> runningList = new ArrayList<>();
        for (HistoricActivityInstance historicActivityInstance : activityInstances) {
            String historicActivityId = historicActivityInstance
                    .getActivityId();
            BpmnModel bpmnModel = Context.getProcessEngineConfiguration().getRepositoryService().getBpmnModel(processDefinitionId); //获取流程xml模型
            FlowNode activity = (FlowNode) bpmnModel.getMainProcess().getFlowElement(historicActivityId); //获取历史节点
//            ActivityImpl activity = definition.findActivity(historicActivityId);//5.X代码

            if (activity != null) {
                if (historicActivityInstance.getEndTime() == null) {
                    // 节点正在运行中  2017-08-09 mjm 为避免会签节点 多个人 部分处理的情况 运行节点图片若先生成会被已完成覆盖 导致看不到红框框
                	//这里收集运行节点  在for循环后面统一绘制 运行中节点
                	runningList.add(historicActivityInstance);
                    ...
                    //12 fax 2017-07-29 由于退回的时候 走了强撤会将 当前任务数据给删除了，导致历史用户任务表数据不存在而空指针 想注释了吧 ，不影响当前绘图 
                   /* if (historicActivityInstance.getTaskId() != null) {
                    ...
                    GraphicInfo flowNodeGraphicInfo = bpmnModel.getGraphicInfo(activity.getId());
                    // 节点已经结束
                    if ("跳过".equals(deleteReason)) {
                        signSkipNode(image, (int)flowNodeGraphicInfo.getX() - this.minX,
                        		(int)flowNodeGraphicInfo.getY() - this.minY,
                                (int)flowNodeGraphicInfo.getWidth(),
                                (int)flowNodeGraphicInfo.getHeight(),
                                historicActivityInstance.getActivityType());
                    } else {
                        signHistoryNode(image, (int)flowNodeGraphicInfo.getX() - this.minX,
                        		(int)flowNodeGraphicInfo.getY() - this.minY,
                                (int)flowNodeGraphicInfo.getWidth(),
                                (int)flowNodeGraphicInfo.getHeight(),
                                historicActivityInstance.getActivityType());
        ...
        //运行中的节点 放到最后统一绘制 保证覆盖到最上面
        if(runningList.size() > 0){
        	for (HistoricActivityInstance historicActivityInstance2 : runningList) {
        		 String historicActivityId = historicActivityInstance2
                         .getActivityId();
        		 BpmnModel bpmnModel = Context.getProcessEngineConfiguration().getRepositoryService().getBpmnModel(processDefinitionId); //获取流程xml模型
                 FlowNode activity = (FlowNode) bpmnModel.getMainProcess().getFlowElement(historicActivityId); //获取历史节点
                 GraphicInfo flowNodeGraphicInfo = bpmnModel.getGraphicInfo(activity.getId());
        		signRunningNode(image,  (int)flowNodeGraphicInfo.getX() - this.minX,
        				(int)flowNodeGraphicInfo.getY() - this.minY, 
                        (int)flowNodeGraphicInfo.getWidth(), 
                        (int)flowNodeGraphicInfo.getHeight(),
                        historicActivityInstance2.getActivityType());
			}
        }
    ...
    public void drawSequenceFlow(BufferedImage image,
        ...
        try {
            ...
            int radius = SEQUENCE_FLOW_RADIUS;
